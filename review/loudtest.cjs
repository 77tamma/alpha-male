...
(async()=>{
  ...
  for (const off of [-300,300,800,1300]) {
    await evaluate(cdp,`window.scrollTo(0,${Math.round(top+off)})`); await sleep(1400);
    const st=await evaluate(cdp,`(()=>{const h=document.getElementById('how'),v=document.getElementById('coupleLoop');
      return {p:getComputedStyle(h).getPropertyValue('--p').trim(),
        vidPaused:v.paused, vidT:+v.currentTime.toFixed(2), vidReady:v.readyState,
        beats:[...document.querySelectorAll('.beat')].map(b=>+getComputedStyle(b).opacity.slice(0,3)),
        formula:[...document.querySelectorAll('.formula span')].map(x=>+getComputedStyle(x).opacity.slice(0,3)),
        overflowX:document.documentElement.scrollWidth-document.documentElement.clientWidth};})()`);
    console.log('off='+String(off).padStart(5), JSON.stringify(st));
    await shot(cdp,`${OUT}/loud-${off}.png`);
  }
  // contrast of the copy over the veiled footage, at its worst
  await evaluate(cdp,`window.scrollTo(0,${Math.round(top+900)})`); await sleep(1500);
  for (const sel of ['.loud .h','.beat h3','.beat p','.formula span']) {
    ...
    await evaluate(cdp,`document.querySelectorAll('.loud .h,.beat h3,.beat p,.formula,.eyebrow').forEach(e=>e.style.visibility='hidden')`);
    await sleep(220);
    ...
    await evaluate(cdp,`document.querySelectorAll('.loud .h,.beat h3,.beat p,.formula,.eyebrow').forEach(e=>e.style.visibility='')`);
    ...
    console.log(sel.padEnd(16), 'over rgb('+px.join(',')+')  =', ratio(col,px).toFixed(2)+':1', ratio(col,px)>=3.5?'PASS':'FAIL');
  }
